...
def create_perfection_game(
    ...
    Args:
        opponents: Nombre d'adversaires IA (3-9 côté UI, max 3 supportés ici)
        difficulty: Nom symbolique (future IA)
...
def _clamp_players(num_players: int) -> int:
    # Le moteur MVP supporte max 4 joueurs
    return max(2, min(4, num_players))


def _compute_board_size(opponents: int) -> int:
    """Calcule une taille de carte approximative."""
    opponents = max(1, opponents)
    base = 10
    return base + min(5, opponents)  # Taille modeste pour MVP


def _skip_inactive_players(state: GameState, max_turns: int) -> GameState:
    """Passe automatiquement les tours des IA inactives."""
    loop_guard = 0
